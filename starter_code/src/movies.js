...
var moviesHtM = movies.slice();

function turnHoursToMinutes(moviesHtM) {
      durations = [];
      averageRate = 0;
      for (i = 0; i < moviesHtM.length; i++) {
            if (moviesHtM[i].duration.length > 5) {
                  let splited = moviesHtM[i].duration.split(" ");
                  let first = splited[0].slice(0, -1) * 60;
                  let second = splited[1].slice(0, -3);
                  moviesHtM[i].duration = parseInt(first) + parseInt(second);
            } else if (moviesHtM[i].duration.length == 5) {
                  moviesHtM[i].duration = parseInt(
                        moviesHtM[i].duration.slice(0, -3)
                  );
            } else {
                  moviesHtM[i].duration = parseInt(
                        moviesHtM[i].duration.slice(0, -1) * 60
                  );
            }
            durations += moviesHtM[i].duration + ",";
      }
}

// Get the average of all rates with 2 decimals

function ratesAverage(moviesHtM) {
      var allRate = 0;
      for (i = 0; i < moviesHtM.length; i++) {
            allRate += parseInt(moviesHtM[i].rate);
      }
      var averageRate = (allRate / moviesHtM.length).toFixed(2);
      return averageRate;
}
...
function dramaMoviesRate(array) {
      ...
      for (let i = 0; i < array.length; i++) {
            if (array[i].genre.includes("Drama")) {
                  allDramaRate += array[i].rate;
                  allDramaNumb += 1;
            }
            return (allDramaRate / allDramaNumb).toFixed(2);
      }
}
...
//       turnHoursToMinutes(moviesHtM);
//       var points = [];
//       for (let i = 0; i < array.length; i++) {
//             points.push(array[i].duration);
//       }
...
//             return points;
//       }
//       return myFunction(points);
// }

// `map`, `reduce`, `filter` y `sort`
